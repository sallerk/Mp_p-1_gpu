...
static int runMain(int argc, char** argv) {
  ...
  try {
    ...
    // P+1 has no bounds model of its own -- it already borrows P-1's B1
    // (todo.md item 1), and reusing the same B2/pairing shape here is the
    // same category of approximation, not a new one. `stages = 1` now means
    // "no stage 2 for either method," a single consistent switch.
    const bool wantPp1Stage2 = cfg.doPP1 && cfg.stage2Mode != STAGE2_OFF && bounds.b2 > bounds.b1;
    ...
    if (cfg.doPP1) {
      ...
      for (u32 seed : cfg.pp1Seeds) {
        ...
        if (wantPp1Stage2 && !gInterrupted.load()) {
          PP1Stage2Result s2 = runPP1Stage2(*gpu, cfg, pr.residue, seed, b1,
          ...
          if (s2.foundFactor) {
            anyFactor = true;
            reportFactors(cfg, s2.factors, b1, bounds.b2, "P+1", seed);
            break;
          }
      ...
      if (!anyFactor) {
        writeResultJson(cfg, "P+1", b1, wantPp1Stage2 ? bounds.b2 : b1, {}, 0);
      }
      if (!cfg.doPM1) {
        printf("\n  appended to %s\n", cfg.resultsFile.c_str());
        return 0;
