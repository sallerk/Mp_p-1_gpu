...
> This is the 1.2 release, kept exactly as it shipped. What changed since is in
...
P+1 now has both stages too: stage 2 catches `q+1` that is B1-smooth apart
from one prime in `(B1, B2]`, exactly like P-1's stage 2, using the same `b2`,
`stages`, `stage2_d` and `stage2_w` keys (P+1 has no bounds/pairing-shape
model of its own yet, so this is the same kind of approximation as already
borrowing P-1's `b1`, not a new one — see Scope below). Checkpointing and
...
- **P+1 currently borrows P-1's bounds and pairing shape** (`b1`, `b2`,
  `stage2_d`, `stage2_w`), all chosen for a different smoothness target.
