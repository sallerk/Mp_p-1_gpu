...
#else
// MSVC has no 128-bit integer type. Of the three only u128 is actually used --
// by BitBucket in state.cpp -- and only for shift, add and truncation to i64,
// so a two-limb stand-in with exactly those operations is enough. i128 and f128
// are deliberately left undefined so any future use fails loudly at compile time
// rather than silently losing range.
struct u128 {
  u64 lo{}, hi{};

  constexpr u128() = default;
  constexpr u128(u64 v) : lo{v} {}
  constexpr u128(u64 lo, u64 hi) : lo{lo}, hi{hi} {}

  // Implicit, to match what native unsigned __int128 does on GCC: state.cpp
  // relies on "bucket.bits == 0" and "data[0] += bucket.bits" compiling as
  // ordinary integer arithmetic. Truncating to the low limb is correct there --
  // both uses are guarded by asserts that the value is 0 or 1.
  constexpr operator u64() const { return lo; }
  ...
  friend u128 operator<<(u128 a, u32 n) { a <<= n; return a; }
  friend u128 operator>>(u128 a, u32 n) { a >>= n; return a; }
};
#endif

// printf/scanf format checking is a GCC/Clang attribute; MSVC has no equivalent.
#ifdef __GNUC__
#define ATTR_FORMAT(KIND, FMT_IDX, ARG_IDX) __attribute__((format(KIND, FMT_IDX, ARG_IDX)))
#else
#define ATTR_FORMAT(KIND, FMT_IDX, ARG_IDX)
#endif
