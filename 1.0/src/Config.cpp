...
bool loadConfig(const string& path, Config& cfg, string& err) {
  ...
  while (fgets(buf, sizeof(buf), f)) {
    ...
    u64 n = 0;
    if (key == "exponent") {
      if (!parseNumber(val, n) || n < 3) { return bad("must be a prime >= 3"); }
      cfg.exponent = u32(n);
    } else if (key == "method") {
  ...
  if (!cfg.exponent) { err = "no 'exponent' given"; return false; }
  if (!cfg.factoredTo) { cfg.factoredTo = defaultFactoredTo(cfg.exponent); }
