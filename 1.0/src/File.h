...
// MSVC has no <unistd.h>; ftruncate/fileno live in <io.h> (see the _chsize
// mapping below).
#include <io.h>
#else
#include <unistd.h>
...
public:
  ...
  int printf(const char *fmt, ...) const ATTR_FORMAT(printf, 2, 3) {
    va_list va;
    va_start(va, fmt);
  ...
  int scanf(const char *fmt, ...) ATTR_FORMAT(scanf, 2, 3) {
    va_list va;
    va_start(va, fmt);
