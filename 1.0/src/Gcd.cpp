...
// File scope so setGcdThreads() can assign it before the anonymous namespace
// that consumes it.
static unsigned gThreadLimit = 0;          // 0 == all hardware threads

u64 gLehmerFastSteps = 0;
u64 gLehmerFallbackDivisions = 0;

void setGcdThreads(unsigned n) { gThreadLimit = n; }
...
// cores is most of the available win.
//
// A global budget keeps the recursion from spawning threads at every level:
// once the machine is saturated the tasks simply run inline. Without that,
// a depth-20 recursion would create thousands of threads.
// ---------------------------------------------------------------------------
...
const size_t PARALLEL_MIN_LIMBS = 2000;
