...
GPU **P-1** factoring of Mersenne numbers `M_p = 2^p - 1`, with both stages, and
P+1 at currently stage 1 only. Windows / MSVC, no external dependencies — no
GMP, no CUDA or OpenCL SDK.
...
| version | download | |
|---|---|---|
| **1.1** | [**Mp_p-1_gpu-1.1-win64.zip**](https://github.com/sallerk/Mp_p-1_gpu/releases/download/v1.1/Mp_p-1_gpu-1.1-win64.zip) | current. Raising B2 on a finished stage 2 now reuses the completed accumulator and walks only the new range, instead of starting over. Start here — see [1.1/README.md](1.1/README.md). |
...
The [1.1/README.md](1.1/README.md) is the manual: `config.txt` keys, the
