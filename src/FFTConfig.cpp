...
FFTConfig::FFTConfig(const string& spec) {
  ...
  // Note the "else" on the second branch: upstream has a bare "if" there, so a
  // one-element spec (e.g. "6.5M") assigned *this and then fell through the
  // chain to the trailing throw, making a bare size spec always fail.
  if (v.size() == 1) {
    ...
    *this = {FFTShape{fft_type, v[0], v[1], v[2]}, parseInt(v[3]), CARRY_AUTO};
    ...
    int c = parseInt(v[4]);
    assert(c == 0 || c == 1);
    // The u32 cast is required: CARRY_KIND -> u32 is a narrowing conversion in
    // a braced-init-list, which MSVC rejects (GCC only warns).
    *this = {FFTShape{fft_type, v[0], v[1], v[2]}, parseInt(v[3]), u32(c == 0 ? CARRY_32 : CARRY_64)};
  } else {
    throw "FFT spec";
  }
}
