...
FFTConfig chooseVerifiedFFT(GpuCommon shared, Queue* q, u32 E,
  ...
  if (!forcedSpec.empty()) {
    ...
    // Say nothing about tuning where a tune could not pay for itself. --tune
    // runs for 15-90 minutes; a 6- or 7-digit exponent is a job of well under
    // a minute, so this was reporting a missing file the reader has no reason
    // to want, above advice to spend an hour speeding up something already
    // finished.
    //
    // What a tune.txt buys is candidate ORDER, and since the search below
    // measures candidates rather than trusting their order, that is worth less
    // than it used to be: at every exponent checked from M786433 to M120000007
    // the untuned path found the same transform a tuned one would have. Above
...
Nat stage2Reference(u32 p, u32 base, const Stage2Plan& plan) {
  ...
  // x^((j+1)^2) = x^(j^2) * x^(2j+1), stepping j by 1. The GPU steps by 2 off
  // x^(4j+4) instead, so the two agree on the identity but not on the chain;
  // the GPU chain is pinned to direct exponentiation separately, by the
  // table-build check below, which is the stricter test of the two.
...
int runStage2Tests(GpuCommon shared, Queue* q, const string& fftSpec) {
  ...
  // The T_j table is built by a second-difference chain rather than one
  // exponentiation per entry. That is a pure speed change -- every T_j must come
  // out bit for bit what direct exponentiation gives -- so test it against
  // exactly that, entry by entry, on the GPU. Direct exponentiation is what
  // shipped through 1.6, which makes this a differential test against code with
  // real production mileage rather than against a reference written alongside it.
  //
  // The shapes below are chosen for the chain's failure modes, not the maths:
  // it walks the odd j and stores only those coprime to D, so what can break is
  // the bookkeeping over a long walk with many skips (w=9), and a D whose
  // radical brings in 11 and changes which j are skipped (D=2310).
  printf("\n  T_j table: second-difference chain vs direct exponentiation\n");
  {
    struct TCase { u32 exponent; u64 b1; u64 b2; u32 d; u32 w; };
    static const TCase TCASES[] = {
      {86599237, 1000000, 1002000, 210,  3},   // realistic exponent
      {859433,      2000,    3000, 210,  9},   // long walk, most j skipped
      {859433,      2000,    3000, 2310, 1},   // different radical
    };
    for (const TCase& c : TCASES) {
      try {
        ...
        const u32 bad = gpu->stage2TableCheck(makeWords(c.exponent, 3), plan);
        const bool ok = (bad == 0);
        check(ok, "every T_j matches direct exponentiation");
        printf("     %s  M%-8u D=%-4u w=%u  %4zu entries, j < %-5llu  %s  (%s)\n",
               ok ? "PASS" : "FAIL", c.exponent, c.d, c.w, plan.jset.size(),
               (unsigned long long) (u64(c.w) * c.d / 2),
               ok ? "all identical" : (std::to_string(bad) + " DIFFER").c_str(),
               fmtDuration(t.at()).c_str());
      } catch (const char* s) {
        printf("     SKIP (%s)\n", s);
      } catch (const std::exception& e) {
