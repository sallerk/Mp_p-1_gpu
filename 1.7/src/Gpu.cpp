...
#define K(name, ...) name(#name, &compiler, profile.make(#name), queue, __VA_ARGS__)
  ...
  // 64
  K(kSubSmall, "etc.cl", "subSmall", 1, "-DSUBSMALL=1"),
  K(kSubWords, "etc.cl", "subWords", hN, "-DSUBWORDS=1"),
...
void Gpu::square(Buffer<Word>& out, Buffer<Word>& in, enum LEAD_TYPE leadIn, enum LEAD_TYPE leadOut, bool doMul3, bool doLL) {
  ...
  // Use CarryFused
    ...
    if (doLL) {
      ...
      // 3*x^2 with the carry still fused. P-1 stage 1 raises the fixed base 3
      // to a huge exponent, so roughly half of all iterations need this; the
      // alternative is dropping to the unfused carry on every set bit.
      carryFusedMul(in_place ? buf1 : buf2, buf1);
    } else {
...
// P-1 stage 1: 3^E mod (2^E-1), with E supplied as little-endian 64-bit limbs.
//
// Left-to-right binary ladder over E's bits. The leading bit is consumed by
// seeding x = 3; every subsequent bit is one squaring, fused with a multiply by
// 3 when the bit is set. That is exactly what the MUL3 carry kernel computes,
// so a set bit costs the same as a clear one.
Words Gpu::powBase3(const vector<u64>& expLimbs, u32 reportEvery,
                    const std::function<bool(u64, u64)>& progress,
                    const Words* resumeFrom, u64 resumeBit,
                    u32 saveEvery,
  ...
  for (size_t bi = startBit; bi-- > 0; ) {
    ...
    // A checkpoint has to read bufData, but with leadOut == LEAD_WIDTH the live
    // value sits in the transform buffers and bufData is stale. So force a full
    // carry on the iteration that precedes a save -- and, when checkpointing is
    // on, also on the iteration that precedes a report check, since that is the
    // only place an interrupt gets noticed. Without this, Ctrl-C between two
    // scheduled saves had nothing valid to write and silently discarded up to
    // saveEvery squarings' worth of work; forcing it here costs one full carry
    // every reportEvery squarings (negligible) instead.
    ...
    enum LEAD_TYPE leadOut =
        (useLongCarry || last || wantSave || (wantCheck && checkpointing)) ? LEAD_NONE : LEAD_WIDTH;

    square(bufData, bufData, leadIn, leadOut, bit, false);
    leadIn = leadOut;
    ++done;

    if (wantSave) {
      queue->finish();
      save(readData(), u64(bi));      // bi == the next bit still to process
    }
...
Words Gpu::lucasV(u32 seed, const vector<u64>& expLimbs, u32 reportEvery,
  ...
  for (size_t bi = startBit; bi-- > 0; ) {
    ...
    if (bit) {
      ...
      squareLL(A, LEAD_NONE, LEAD_NONE);        // A = V_{2k} = A^2 - 2
      B << T;                                   // B = V_{2k+1}
    }
    ++done;

    // Both A and B are always fully carried here (the ladder uses LEAD_NONE
    // throughout), so a checkpoint can read them without any extra work -- on
    // interrupt as much as on a scheduled save.
    if (saveEvery && save && done % saveEvery == 0) {
      queue->finish();
      save(readAndCompress(A), readAndCompress(B), u64(bi));
    }
    if (reportEvery && done % reportEvery == 0) {
      queue->finish();
      if (!progress(done, total)) {
        if (saveEvery && save) { save(readAndCompress(A), readAndCompress(B), u64(bi)); }
        break;
      }
    }
  }
...
//
// This is what B1 extension needs: reuse a completed stage-1 residue x as the
// base and raise it to R = E(newB1)/E(oldB1). Unlike powBase3 the multiplier is
// not the constant 3, so it cannot be folded into the carry by the MUL3 kernel
// -- each set bit costs a real multiply. That makes an extension about 1.5x the
// per-bit cost of a fresh ladder, which is inherent to a general base and still
// far cheaper than redoing the whole of stage 1.
Words Gpu::powResidue(const Words& base, const vector<u64>& expLimbs,
                      u32 reportEvery,
                      const std::function<bool(u64, u64)>& progress,
                      const Words* resumeFrom, u64 resumeBit,
                      u32 saveEvery,
  ...
  if (!nBits) { return base; }                 // R == 0 cannot happen; R == 1 is nBits == 1

  // The base has to stay resident in transform form across the whole ladder,
  // and square() uses buf1..buf3 as scratch, so it needs a buffer of its own.
  Buffer<double> bufBase{timeBufVect, queue,
                         TOTAL_DATA_SIZE(fft, WIDTH, fft.shape.middle, SMALL_H,
                                         in_place, pad_size)};
  writeIn(bufCheck, base);
  if (!in_place) {
    fftP(buf2, bufCheck);
    fftMidIn(bufBase, buf2);
  } else {
    fftP(bufBase, bufCheck);
  ...
  for (size_t bi = startBit; bi-- > 0; ) {
    ...
    if (bit) {
      mul(bufData, bufBase, buf2, buf3, false);
      leadIn = LEAD_NONE;
    }
    ++done;
...
void Gpu::powSmall(Buffer<Word>& out, Buffer<Word>& base, u64 exp) {
  assert(exp > 0);
  out << base;
  for (int b = 62 - std::countl_zero(exp); b >= 0; --b) {
    square(out);
    if ((exp >> b) & 1) { modMul(out, base); }
  }
}

void Gpu::renormalize(Buffer<Word>& io, Buffer<Word>& one) { modMul(io, one); }

// V_n(base, 1) mod (2^E-1). See the header for the contract.
Words Gpu::lucasVResidue(const Words& base, u64 n) {
  if (n == 0) { return makeWords(E, 2); }

  auto bufs = makeBufVector(5);
  Buffer<Word>& A       = bufs[0];   // V_k
  Buffer<Word>& B       = bufs[1];   // V_{k+1}
  Buffer<Word>& T       = bufs[2];
  Buffer<Word>& one     = bufs[3];
  Buffer<Word>& baseBuf = bufs[4];   // base, held on the GPU: kSubWords needs a
                                     // device buffer, not the host Words above.

  writeIn(one, makeWords(E, 1));
  writeIn(baseBuf, base);

  // Leading bit consumed by starting at k = 1: A = V_1 = base, B = V_2 = base^2-2.
  A << baseBuf;
  B << baseBuf;
  squareLL(B, LEAD_NONE, LEAD_NONE);

  const size_t nBits = std::bit_width(n);
  for (size_t bi = nBits - 1; bi-- > 0; ) {
    const bool bit = (n >> bi) & 1;

    // V_{2k+1} = A*B - base, formed before either input is overwritten.
    // Unlike lucasV's kSubSmall(-seed), this subtracts a full residue, so the
    // result is renormalised before it can be squared on a later bit -- see
    // the header comment on why that precaution exists here and not in lucasV.
    T << A;
    modMul(T, B);
...
void Gpu::buildStage2Table(vector<Buffer<Word>>& T, Buffer<Word>& xBuf,
  ...
  // d, which is what makes every j coprime to d odd -- if that ever stopped
  // holding, the walk would step past a j it was supposed to store and spin.
  assert(plan.d % 2 == 0);

  auto chain = makeBufVector(3);
  Buffer<Word>& u = chain[0];   // x^(j^2)
  Buffer<Word>& s = chain[1];   // x^(4j+4)
  Buffer<Word>& k = chain[2];   // x^8, the constant step for s
...
u32 Gpu::stage2TableCheck(const Words& x, const Stage2Plan& plan) {
  const size_t J = plan.jset.size();
  assert(J > 0);

  auto T = makeBufVector(u32(J));
  auto scratch = makeBufVector(2);
  Buffer<Word>& xBuf = scratch[0];
  Buffer<Word>& ref  = scratch[1];
  ...
  for (size_t i = 0; i < J; ++i) {
    const u64 j = plan.jset[i];
    powSmall(ref, xBuf, j * j);
    if (!isEqual(T[i], ref)) { ++bad; }
  }
  return bad;
}

// P-1 stage 2. See the header for the contract and Stage2Plan.h for the maths.
Words Gpu::stage2(const Words& x, const Stage2Plan& plan, u32 reportEvery,
                  const std::function<bool(u64, u64)>& progress,
  ...
  // overflow here would silently compute the wrong thing.
  const u64 mdLast = plan.mLast * D;
  if (mdLast > 4294967295ull) { throw std::runtime_error("stage 2: B2 too large for 64-bit exponents"); }

  if (mulRoeOut) { wantROE = ROE_SIZE; readROE(); }

  auto scratch = makeBufVector(6);
  Buffer<Word>& xBuf = scratch[0];
  Buffer<Word>& acc  = scratch[1];
  Buffer<Word>& A    = scratch[2];   // x^((m*D)^2)
  Buffer<Word>& S    = scratch[3];   // x^(D^2 * (2m+1)), the step from A_m to A_m+1
  Buffer<Word>& C    = scratch[4];   // x^(2*D^2), the step from S_m to S_m+1
  Buffer<Word>& diff = scratch[5];

  writeIn(xBuf, x);

  // T_j = x^(j^2), by a second-difference chain stepping j by 2:
  //
  //   x^((j+2)^2) = x^(j^2) * x^(4j+4)        x^(4(j+2)+4) = x^(4j+4) * x^8
  //
  // so carrying (u, s) = (x^(j^2), x^(4j+4)) forward costs two multiplies per
  // step of 2 in j -- one multiply per unit of j, regardless of how many j are
  // stored. d is even, so every j coprime to d is odd and this chain lands on
  // all of them; jset[0] is always 1, where the chain starts.
  //
  // The direct powSmall(x, j*j) per entry this replaces cost about 2*log2(j)
  // multiplies EACH. At D=2310, w=9 that is 2160 entries of ~36 = ~78000
  // multiplies to build a table the main loop then uses for only ~85000. The
  // chain does the same work in ~10400. Measured end to end on M5378909 at
  // that shape, B1=150000 B2=2000000: stage 2 fell from 33s to 24s, with the
  // accumulator bit for bit identical and the multiply count unchanged --
  // every second of it setup that is no longer paid for.
  //
  // These stay as WORDS and the difference A_m - T_j is formed in word space.
  // Holding the table in middle-transform form instead lets the difference be
  // formed AFTER transforming (fftP and fftMidIn are linear), removing one
  // forward transform per accumulator multiply. That was built and measured:
  // the multiply fell from 1.56x a squaring to 1.40x, but a transform buffer is
  // 1.5x a Word buffer, so fewer T_j fit and the pairing shape worsened from
  ...
  powSmall(C, xBuf, 2 * D * D);

  u64 mStart = plan.mFirst;
  u64 jStart = 0;
  u64 done = 0;

  if (resume) {
    // A and S are the whole point of saving: recomputing A_m for a large m by
    // exponentiation would work, but S would still have to be rebuilt, and
    // trusting the saved pair keeps resume identical to an uninterrupted walk.
    writeIn(acc, resume->acc);
    writeIn(A, resume->a);
    writeIn(S, resume->s);
    mStart = resume->m;
    jStart = resume->jIdx;
    ...
    const u64 m0 = plan.mFirst;
    powSmall(A, xBuf, (m0 * D) * (m0 * D));
    powSmall(S, xBuf, D * D * (2 * m0 + 1));

    // acc starts at 1 rather than at the first difference on purpose: every
    // accumulator value is then the output of a carry pass, so only ONE operand
  ...
  // Capture the position BEFORE the slot about to be processed, so a resume
  // repeats nothing and skips nothing.
  auto capture = [&](u64 m, u64 jIdx) {
    Stage2Pos p;
    p.m = m;
    p.jIdx = jIdx;
    p.done = done;
    p.acc = readAndCompress(acc);
    p.a = readAndCompress(A);
    p.s = readAndCompress(S);
    return p;
  };

  for (u64 m = mStart; m <= plan.mLast && !stop; ++m) {
    for (size_t i = size_t(m == mStart ? jStart : 0); i < J; ++i) {
      if (!plan.slot(m, i)) { continue; }
      kSubWords(diff, A, T[i]);
      // readAndCompress carries the words on the CPU, so writing the result
      // straight back yields the identical value in normalised form.
      if (normalizeDiff) { writeIn(diff, readAndCompress(diff)); }
      modMul(acc, diff);
      ++done;
  ...
  if (mulRoeOut) {
    auto [squareRoe, mulRoe] = readROE();
    (void) squareRoe;
    *mulRoeOut = mulRoe;
    wantROE = 0;
  }
  return readAndCompress(acc);
}

// P+1 stage 2. See the header for the contract and derivation.
Words Gpu::pp1Stage2(const Words& y1, const Stage2Plan& plan, u32 reportEvery,
                     const std::function<bool(u64, u64)>& progress,
                     RoeInfo* mulRoeOut, bool normalizeDiff,
                     const Stage2Pos* resume, u32 saveEvery,
                     const std::function<void(const Stage2Pos&)>& save,
                     Stage2Pos* stoppedAt, const Words* accSeed,
                     bool skipRenormalize) {
  const u64 J = plan.jset.size();
  const u64 D = plan.d;
  assert(J > 0);
  // A resumed walk already has the seed folded into its saved accumulator;
  ...
  auto scratch = makeBufVector(7);
  Buffer<Word>& yBuf  = scratch[0];
  Buffer<Word>& acc   = scratch[1];
  Buffer<Word>& A     = scratch[2];   // A_curr = V_(m*D)(y1,1)
  Buffer<Word>& Aprev = scratch[3];   // A_prev = V_((m-1)*D)(y1,1)
  Buffer<Word>& VD    = scratch[4];   // V_D(y1,1) -- the constant block step
  Buffer<Word>& one   = scratch[5];
  Buffer<Word>& diff  = scratch[6];

  writeIn(yBuf, y1);
  writeIn(one, makeWords(E, 1));
  // Not checkpointed: cheap to recompute from scratch on every resume, same
  // precedent as stage2()'s own per-run constant C.
  writeIn(VD, lucasVResidue(y1, D));

  // T_j = V_j(y1,1), built by ONE incremental pass over the linear indices
  // 1..jmax (T_(n+1) = y1*T_n - T_(n-1)), rather than P-1's independent
  // per-entry ladder -- P+1's table indices are sequential, not squared, so a
  // single walk suffices. Tprev/Tcur are PERSISTED and reused across steps
  // (unlike the per-slot `diff` below), so every step's difference is
  // renormalised before becoming the next step's input -- see Gpu::renormalize.
  auto T = makeBufVector(u32(J));
  {
    const u64 jmax = u64(plan.w) * D / 2;
    std::vector<int> idxOf(size_t(jmax), -1);
    for (size_t i = 0; i < J; ++i) { idxOf[plan.jset[i]] = int(i); }

    auto tb = makeBufVector(2);
    Buffer<Word>& Tprev = tb[0];   // T_{n-1}
    Buffer<Word>& Tcur  = tb[1];   // T_n

    writeIn(Tprev, makeWords(E, 2));   // T_0 = V_0 = 2
    writeIn(Tcur, y1);                 // T_1 = V_1 = y1
    if (idxOf[1] >= 0) { T[size_t(idxOf[1])] << Tcur; }

    for (u64 n = 1; n < jmax - 1; ++n) {
      diff << Tcur;
      modMul(diff, yBuf);
      kSubWords(diff, diff, Tprev);
      if (!skipRenormalize) { renormalize(diff, one); }
      Tprev << Tcur;
      Tcur << diff;
      const int idx = idxOf[size_t(n + 1)];
      if (idx >= 0) { T[size_t(idx)] << Tcur; }
    }
  }
  ...
  if (resume) {
    writeIn(acc, resume->acc);
    writeIn(A, resume->a);
    writeIn(Aprev, resume->s);
    mStart = resume->m;
    jStart = resume->jIdx;
    done = resume->done;
  } else {
    const u64 m0 = plan.mFirst;
    // buildStage2Plan's own invariant (w*d/2 <= b1 <= m0*d) forces m0 >= 1, so
    // (m0-1)*D is >= 0 and commonly exactly 0 -- lucasVResidue's n==0 case,
    // not a rare corner one.
    writeIn(A, lucasVResidue(y1, m0 * D));
    writeIn(Aprev, lucasVResidue(y1, (m0 - 1) * D));
    writeIn(acc, accSeed ? *accSeed : makeWords(E, 1));
  }
  ...
  for (u64 m = mStart; m <= plan.mLast && !stop; ++m) {
    for (size_t i = size_t(m == mStart ? jStart : 0); i < J; ++i) {
      if (!plan.slot(m, i)) { continue; }
      kSubWords(diff, A, T[i]);
      // Single-use, exactly like P-1's diff: consumed immediately by the one
      // modMul below, whose own carry pass normalises it -- no renormalize
      // needed here (see Gpu::renormalize's comment for the distinction).
      if (normalizeDiff) { writeIn(diff, readAndCompress(diff)); }
      modMul(acc, diff);
      ++done;
    ...
    if (m < plan.mLast && !stop) {
      // A_next = A_curr*V_D - A_prev. Persisted and reused as a future block's
      // A_prev, so (unlike the per-slot diff above) this must be renormalised.
      diff << A;
      modMul(diff, VD);
      kSubWords(diff, diff, Aprev);
      if (!skipRenormalize) { renormalize(diff, one); }
      Aprev << A;
      A << diff;
      done += 2;   // matches the "2 modMuls per block" cost, same accounting P-1 uses for A*=S, S*=C
    }
  }
