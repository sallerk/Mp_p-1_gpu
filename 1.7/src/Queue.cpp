...
void Queue::waitForMarkerEvent() {
  ...
  // Flush before polling. Enqueuing a marker does not submit it to the device;
  // on nVidia the queue is only guaranteed to be submitted by a flush or a
  // finish. Queue::finish() calls this function BEFORE ::finish(), so without
  // an explicit flush the loop below can poll an event that was never
  // submitted -- sleeping forever at ~0% CPU with the GPU idle.
  //
  // Upstream survives this because its own loops keep enqueuing, and queue
  // pressure eventually forces a submit. A caller that enqueues a batch and
  // then waits -- exactly what a P-1 ladder with periodic progress reporting
  // does -- deadlocks instead.
  ::flush(get());
